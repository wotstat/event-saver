...
function getQuert(table: string) {
  return `
  alter table ${table} add column modVersion_parts Array(String) materialized extractAllGroups(modVersion, '^(\d+)\.(\d+)\.(\d+)(?:\.(\d+))?(?:-([0-9A-Za-z-.]+))?$')[1] comment 'Части версии мода major.minor.patch.revision-identifier';
